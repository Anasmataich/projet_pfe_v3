...
function getDocVersion(doc: Document): number {
  // ✅ support multiple possible backend fields without breaking TS types
  const d: any = doc;
  const v =
    d.version ??
    d.currentVersion ??
...
export function DocumentCard({ document: doc, onDownload }: Props) {
  ...
  const createdAt = safeDate((doc as any).createdAt ?? (doc as any).created_at);
    ...
    >
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-slate-500 mt-0.5">{(doc as any).category ?? '—'}</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <span className={`${CONF_BADGE[(doc as any).confidentialityLevel] ?? 'badge-gray'} inline-flex items-center gap-1`}>
          <Lock className="h-3 w-3" />
          {(doc as any).confidentialityLevel ?? '—'}
        </span>

        {version > 1 && <span className="badge-purple">v{version}</span>}
      </div>
